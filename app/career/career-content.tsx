...
const employeeAvatarFallbacks = [
  "/assets/figma/career/02.png",
  "/assets/figma/about/05.png",
  "/assets/figma/about/06.png",
  "/assets/figma/about/08.png",
];
    ...
    <>
      ...
      <motion.section initial={{ opacity: 0, scale: 0.98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="career-hero container">
        <ContentImage
          ...
          alt={data.hero.title || ""}
          priority
          decorativeFallback="/assets/figma/career/05.jpeg"
        />
      </motion.section>
      ...
      <section className="employee-section"><div className="container">
        ...
        <div className="testimonials-wrapper">
          {feedback.length ? [feedback.slice(0, 2), feedback.slice(2)].map((row, rowIndex) => (
            <div className={`testimonials-row ${rowIndex ? "offset-row" : ""}`} key={rowIndex}>
              {row.map((item, index) => {
                const fallbackAvatar = employeeAvatarFallbacks[(rowIndex * 2 + index) % employeeAvatarFallbacks.length];
                return (
                  <motion.article
                    whileHover={{ y: -4 }}
                    className={rowIndex === 0 && index === 0 ? "feedback-card dark-card" : "feedback-card light-card"}
                  ...
                  >
                    <div className="feedback-user">
                      <ContentImage
                        ...
                        decorativeFallback={fallbackAvatar}
                      />
                      <strong>{item.customer_name}</strong>
                      <span className="stars">{"★".repeat(Math.max(0, Math.min(5, item.rating || 0)))}</span>
                    </div>
                    <p>{item.testimonial}</p>
                    <div className="card-footer-row">
                      <span>{item.customer_role}</span>
          ...
          )) : <EmptyContent message="Employee feedback is not available from the API." />}
        </div>
      </div></section>
