    ...
    <>
      ...
      <section className="catalog container">
        <div className="tab-row">
          <Link className="active" href="/services">{data.page.tabs?.services || "Services"}</Link>
          <Link href="/products">{data.page.tabs?.products || "Products"}</Link>
        </div>
            ...
            return (
              <motion.article initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className={isOpen ? "open" : ""} key={service.slug}>
                <button type="button" onClick={() => setOpen(isOpen ? null : index)} aria-expanded={isOpen}>
                  ...
                  <strong className="catalog-title">{service.title}</strong>
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="catalog-details">
                      {(service.full_description || service.short_description) && <p className="catalog-description">{service.full_description || service.short_description}</p>}
                      {features.length > 0 && (
